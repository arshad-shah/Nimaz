...
> - **16-line renderer wired behind a temporarily-off seam** (sub-task 5/7 of #263). The line-accurate renderer (`MushafLineLayout` + `MushafLinePage`) is integrated into the reader pager via the `ReaderMushafPage` helper, gated on `QuranReaderUiState.use16LineLayout`. That flag defaults `false` and is **not yet bound to any setting**, so the 16-line branch is unreachable at runtime until sub-task 6/7 (#270) adds the DataStore key + `QuranSettingsScreen` toggle and makes the pager's `totalPages`/page-jump logic script-aware (604 vs 548). This is a deliberate staged seam, not dead code: 6/7 flips the flag and completes the script-aware ayah/page sourcing. The renderer itself is fully covered by Compose previews. See `SUBSYSTEMS.md` §5.
