...
| Mushaf pagination not script-aware (#325) | `MushafScript` supplied only a page *count*; every page→content mapping (the Page tab's juz table ending in a literal `604`, `ayahs.page`, `surahs.start_page`, a third copy of the Madani juz table in `QuranSurahListItem`) stayed Madani-only, so the 16-line layout still listed 604 tiles and khatam page progress/marking hit the wrong ayahs. The pure domain model `MushafPagination` is now the single source of truth for `totalPages`, page↔ayah lookups and juz page spans; it is derived per edition (`ayahs.page` for a flowed edition vs that edition's `mushaf_layouts` rows) and re-derived reactively when the setting changes. The duplicate juz page tables were deleted. Pinned by `MushafPaginationTest`, `QuranPageGridTest`, `QuranJuzGridTest`. |
| Quran content catalogues in presentation | Three of the five Quran content axes had no domain home: the translation picker hardcoded `listOf("Sahih International" to "sahih_international")` in `QuranSettingsScreen`, `SelectReciterScreen` declared `popularReciters` inline, and reciter metadata was in fact **triplicated** across that list, `QuranAudioManager.RECITER_CDN_MAP` and its `getReciterDisplayName` `when` — a fourth partial copy lived in `QuranSettingsScreen`. They had already drifted: five reciters were streamable but never offered, the settings row rendered raw ids like `"hudhaify"` for the six reciters its `when` omitted, and one reciter's name was spelled two ways. `domain/model/quran/catalogue/QuranEditions` is now the single source of truth for translations, mushaf layouts, tafseers, reciters and fonts; asset/CDN bindings live in `data` (`QuranContentAssets`) and `fontId → FontFamily` in `presentation` (`QuranArabicFont`), with `QuranEditionRegistryTest` failing the build if the id sets diverge. `MushafScript` was deleted in favour of `MushafLayoutEdition`; stored preference values are resolved through `legacyKeys`, never rewritten. See `docs/adr/ADR-001..003` and `docs/quran/content-registry.md`. |
| Per-edition Mushaf layout table | `mushaf_layout_indopak16` named its edition in the table itself, so 14 references across 6 DAO methods, a seeder and a migration would have been duplicated per new layout. Replaced by a generic `mushaf_layouts` with a `layout_id` discriminator (`MIGRATION_18_19`, ADR-001); DAO methods take `layoutId`, the seeder is edition-agnostic and driven by the asset bindings, and `MushafLayoutFidelityTest` runs its invariants over every catalogue layout rather than the one. A new line-accurate edition is now data only. |
...
| 11 | Quran / 16-line Mushaf | **Sajda & rukūʿ markers are not shown in the 16-line view.** The shipped `text_indopak` and `mushaf_layout_indopak16` carry no sajda (۩) or rukūʿ (۞) glyphs or line types (verified in 7/7, #271), so `MushafLineLayout` renders the printed word glyphs faithfully but overlays no sajda/rukūʿ medallions. Sajda metadata still lives on `ayahs.sajda_type` and is surfaced by the ayah-keyed reader only. | Regenerate the IndoPak assets with sajda/rukūʿ spans (or a `sajda`/`ruku` `line_type`), map them in `MushafLayoutMapper`, and draw a marker in `MushafLineLayout`; extend `MushafLayoutFidelityTest` to pin the known sajda ayahs. Needs asset regeneration + visual review. |
| 12 | Quran / 16-line Mushaf | **A raw page number is not equivalent across Mushaf editions.** A page `Int` means a different slice of the Quran in the 604-page Madani scheme vs. the 548-page IndoPak-16 scheme (unrelated pagination). In-app navigation is safe as of #325: "Continue reading" resolves by surah/ayah (`ContinueReadingCard.onClick` → `onNavigateToQuranAyah(lastSurah, lastAyah)`, `lastReadPage` is display-only), and every in-app page surface — the Page tab grid, its juz sections, the surah page ranges, the jump-to-page field and the reader's page content — now resolves through the active edition's `MushafPagination` rather than the Madani tables. The one real gap is `AnnouncementRoutes.parameterisedAnnouncementRoute`'s `quran/page/N` deep link: it validates against `QuranEditions.maxTotalPages` (604) and the reader then clamps to the active edition's count, so it can't crash, but a server-sent page deep link can land the reader on unrelated content if the user's active script differs from the one the link was authored against. Accepted as v1 scope — announcement payloads are first-party/curated, not user input. | If this becomes user-facing (e.g. shared deep links), anchor `quran/page/N` by surah/ayah instead of raw page, or tag the page number with its edition in the route. |

> **Accepted patterns (NOT deviations):**
> - **16-line IndoPak data ships as seeded JSON assets, not in the prepackaged DB** (sub-task 2/7 of #263). The IndoPak ayah text + 548-page layout are populated at runtime by `QuranIndopakSeeder` from `assets/quran/*.json`, and `MIGRATION_17_18` only creates the empty `ayahs.text_indopak` column + `mushaf_layout_indopak16` table. The alternative — regenerating `assets/database/nimaz_prepopulated.db` — was rejected: it is a ~147 MB Git-LFS blob that `createFromAsset` copies **only on fresh install**, so baking the data in would (a) never reach existing installs and (b) grow the LFS asset by tens of MB. The JSON assets add only **~0.75 MB compressed** to the APK and reach both fresh installs and upgraders via the same seeder path used by Dua/Help/Qaida. `generate_database.py` is still kept schema-complete (it can ingest the JSON if the DB is ever regenerated from scratch), but the shipped asset is intentionally left untouched. The seeding **trigger** landed in sub-task 4/7: `QuranRepositoryImpl.getMushafPageLayout(page)` calls `indopakSeeder.seedIfNeeded()` before reading, so the seed runs at most once, on first use of the 16-line view. The line-accurate read path (`getMushafLayoutByPage` → `MushafLayoutMapper` → `MushafPageLayout` domain model → `GetMushafPageLayoutUseCase`) keeps the layers clean: the pure grouping/word-reconstruction lives at the data layer and the domain model carries no Room types. See `SUBSYSTEMS.md` §5/§7.
> - **16-line renderer, now user-selectable & persisted** (sub-tasks 5/7 + 6/7 of #263, #270). The line-accurate renderer (`MushafLineLayout` + `MushafLinePage`) is integrated into the reader pager via the `ReaderMushafPage` helper, gated on `QuranReaderUiState.useLineLayout`. As of 6/7 that gate is driven by a persisted preference: `SettingsRepository.quranMushafScript` (DataStore key `quran_mushaf_script`, a `QuranEditions.mushafLayouts` id, default `madani`) is folded into `QuranViewModel` state, where `useLineLayout` and `totalPages` are **computed from** `mushafScript` (single seam — no drift between "which renderer" and "how many pages"). The "Mushaf Script" dropdown in `QuranSettingsScreen` (`SettingsEvent.SetMushafScript`) writes it; the reader's pager count, dual-page spread count, and the Quran-home jump-to-page all read `state.mushafScript.totalPages` (604 vs 548), and deep-link page bounds validate against `QuranEditions.maxTotalPages`. It stays **off by default**, so the Uthmani/604 view is unchanged unless the user opts in. The renderer is covered by Compose previews and, as of 7/7 (#271), Robolectric render tests (`MushafLinePageTest`), a data-fidelity suite over the shipped assets (`MushafLayoutFidelityTest`), and a generated per-page pass/fail sheet (`docs/quran/16-line-fidelity-sheet.md`). See `SUBSYSTEMS.md` §5/§6.
