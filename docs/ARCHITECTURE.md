...
> - **Mushaf editions and Quran translations ship as seeded JSON assets, not in the prepackaged DB** (sub-task 2/7 of #263, extended when the catalogue grew to 4 editions + 15 translations). Each edition's glyph text + layout, and each translation's verses, are populated at runtime by `MushafLayoutSeeder` / `QuranTranslationSeeder` from `assets/quran/`, and the migrations only create the empty tables. The alternative — regenerating `assets/database/nimaz_prepopulated.db` — was rejected: it is a ~147 MB Git-LFS blob that `createFromAsset` copies **only on fresh install**, so baking the data in would (a) never reach existing installs and (b) grow the LFS asset by tens of MB. The JSON assets add only **~0.75 MB compressed** to the APK and reach both fresh installs and upgraders via the same seeder path used by Dua/Help/Qaida. `generate_database.py` is still kept schema-complete (it can ingest the JSON if the DB is ever regenerated from scratch), but the shipped asset is intentionally left untouched. The seeding **trigger** landed in sub-task 4/7 and is now per item: `QuranRepositoryImpl.getMushafPageLayout(page, script)` calls `mushafSeeder.seedIfNeeded(script)` and every translated read resolves through `seededTranslationId(...)`, so an edition or translation is seeded at most once, on first use — not all 19 up front. The line-accurate read path (`getMushafLayoutByPage` → `MushafLayoutMapper` → `MushafPageLayout` domain model → `GetMushafPageLayoutUseCase`) keeps the layers clean: the pure grouping/word-reconstruction lives at the data layer and the domain model carries no Room types. See `SUBSYSTEMS.md` §5/§7.
