...
**15 translations across 11 languages ship** — English (Saheeh International *default*, Yusuf Ali,
Pickthall, Clear Qur'an/Itani), Urdu (Maududi, Jalandhry), Indonesian, Turkish, French, Bengali,
Hindi, Spanish, Russian, Malay, German.

They are **not** in the prepopulated DB (only Saheeh International ever was). Each is a compact
asset at `app/src/main/assets/quran/translations/<id>.json`, generated by
`nimaz-pro-data/scripts/download_translations.py` from the Al Quran Cloud API (which republishes
the Tanzil corpus), and seeded into the `translations` table the first time it is selected.

The asset is a **positional array** — index `i` holds global ayah id `i + 1` — which is ~⅓ smaller
than an array of objects. That is only safe because the generator hard-fails unless the upstream
edition aligns with `ayahs.json` verse for verse; it also rejects empty verses and wrong counts.

```json
{"translationId":"en_pickthall","contentVersion":1,"source":"…","texts":["In the name of Allah…", "…"]}
```

Sizes: ~750–950 KB for Latin-script languages, ~1.4–2.3 MB for Urdu/Bengali/Hindi; **~18 MB raw,
~4 MB compressed** for all 15.
...
> **Status: data acquired, validated, and in-repo — now for three editions.** The acquisition was
> generalised out of the 16-line-only script into
> `nimaz-pro-data/scripts/download_mushaf_layout.py`, which can fetch any registered Quranic
> Universal Library (QUL) layout. `download_indopak_mushaf_data.py` remains as a thin wrapper;
> re-running the generalised script reproduced the original 16-line files **byte for byte** (all
> 6,236 ayahs, all 13,970 layout rows).
>
> | Script key | QUL resource | Pages | Lines | Text source |
> |---|---|---|---|---|
> | `INDOPAK_16` | 11 — Indopak 16 lines (Taj company) | 548 | 16 | `INDOPAK` |
> | `INDOPAK_15` | 12 — Indopak 15 lines (Qudratullah) | 610 | 15 | `INDOPAK` |
> | `INDOPAK_13` | 313 — Indopak 13 lines (Taj company) | 847 | 13 | `INDOPAK_13` |
>
> Files are `json/mushaf/<text_source>_text.json` (glyph text, **one per text source**) and
> `json/mushaf/<script>_layout.json` (page/line map, one per edition), mirrored into
> `app/src/main/assets/quran/mushaf/`. `INDOPAK_16` and `INDOPAK_15` were verified to have
> byte-identical glyph text for all 6,236 ayahs, so they share one text asset and the 15-line
...
`nimaz-pro-data/json/mushaf/<text_source>_text.json` — one row per existing ayah id. `words[]` is
indexed 1:1 with the layout's word positions, and `text == " ".join(words)`:
```json
[
  {
    "ayah_id": 1,
    "text": "بِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِیْمِ ۟",
...
`nimaz-pro-data/json/mushaf/<script>_layout.json` — one row per line-segment (a line may hold one
...
- [x] Every edition's text source covers all 6,236 ayah ids (ids match `AyahEntity.id`; no missing/extra/empty).
...
the shipped `assets/quran/mushaf/` files directly, for every edition, and re-asserts every
...
**Schema (`NIMAZ_DATABASE_VERSION = 20`).** `MIGRATION_17_18` originally added an
`ayahs.text_indopak` column plus a `mushaf_layout_indopak16` table — a shape that could hold only
one edition. `MIGRATION_19_20` generalised it (in `NimazDatabase.kt`):
- **`mushaf_ayah_texts`** — glyph text keyed by `(text_source, ayah_id)`. A *text source* is the
  script an edition sets its words in; editions with identical glyphs **share** one, so
  `INDOPAK_16` and `INDOPAK_15` both read `INDOPAK` while `INDOPAK_13` carries its own.
- **`mushaf_layout_lines`** — one row per line-segment, mirroring each `<script>_layout.json` 1:1
  (`id` autoincrement PK, `script`, `page`, `line`, `line_type`, `surah_id`, `ayah_id` = global id
  or null, `first_word_position`, `last_word_position`), indexed on `(script, page, line)` and
  `(script)`.

`mushaf_layout_indopak16` is dropped and `ayahs.text_indopak` is nulled (kept as an inert column —
dropping one in SQLite means rebuilding a 6,236-row table for no gain). Adding an edition is now
**data, not schema**: one `MushafScript` entry plus its generated assets.

The glyph text is **not** duplicated into the layout table — the rendering/data layer (4/7)
reconstructs each line's words by slicing the matching `mushaf_ayah_texts.text` with the stored
inclusive positions. This is lossless: the 1/7 data guarantees `text == " ".join(words)` with no
...
would grow the LFS blob by tens of MB. Instead the JSON files ship verbatim as bundled Android
assets under `app/src/main/assets/quran/mushaf/` (all three editions ~12 MB raw, well under 1 MB
compressed each thanks to the shared text source) and are populated at runtime by
`MushafLayoutSeeder` — idempotent, version-gated per edition on the `quran_mushaf_versions`
DataStore set, and run only when an edition is first displayed — exactly as Dua/Help/Qaida content
is seeded. The same applies to the 15 Quran translations under `quran/translations/`
(`QuranTranslationSeeder`). Fresh installs and upgraders both converge on the same data. See
`docs/SUBSYSTEMS.md` §5/§7 and `docs/ARCHITECTURE.md` §9.

**`generate_database.py` is kept schema-complete** — it creates `mushaf_ayah_texts` and
`mushaf_layout_lines` (and the translations unique index) but leaves them **empty**, because that
content travels in the APK assets rather than the DB. The shipped LFS asset is intentionally left
untouched.
