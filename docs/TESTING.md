...
| Which day the screen is on, and how it says so | `PrayerTimesScreenTest` | the relative label and the "Today" pill that renders only `if (!isToday)`. A screen that loses the pill strands a reader on a day they browsed to |
| Reaching another day | same | the week rail selects one, and the month button opens the picker and reports the choice. The rail spans ±3 days, so the button is the only route to anything further |
| Swiping between days | same | the gesture, which is the one people actually use: a drag threshold read with the wrong sign pages backwards |
| What the day card leads with | same | the window the reader is inside on today, the daylight on any other day. "You are in the window of Dhuhr" is a sentence that does not exist on a day that is not today |
| That no row is tappable | same | Prayer Times answers *when*; a row that looked tappable would promise logging the screen no longer performs |
| That no event writes a prayer | `PrayerTimesViewModelTest` | asserted over *every* event rather than a removed one, so an event that writes fails here rather than shipping. This screen was #359's third tracking site, and it wrote a binary `PRAYED`/`NOT_PRAYED` that flattened a prayer recorded as `LATE` |
| That the screen actually draws | `PrayerTimesScreenRenderTest` | both themes and a browsed day written to `build/reports/prayerTimes/`, failing on a blank bitmap. Four defects in this screen — doubled status-bar inset, a figure stated three times, an accent card swallowing the accent-drawn arc, and cards toned to the background colour — passed every assertion above and were caught by looking |
