...
> **Home and the audio stack have since left**, for `:feature:home` and `:core:audio`, and with
> them the rows below marked *(now `:feature:home`)* and *(now `:core:audio`)* — 45 test classes
...
> it measured the ASM-transformed classes instead of the compiler output. Any module with an
> `@AndroidEntryPoint` class a unit test constructs has to set
> `nimazCoverage.measureTransformedClasses`.
