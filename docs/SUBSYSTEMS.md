...
**Playing from one verse.** Everything that starts a recitation at a particular verse — the mushaf's ayah tooltip, the verse action sheet's "Play from here", the bar's play button — sends one `QuranEvent.PlayAyahAudio`, and `QuranViewModel.playAyahAudio` decides what playlist that verse sits in. It prefers the **reader's own** `ayahs`, so playing from a page plays that page and playing from a juz carries on through the juz. That list frequently does not hold the verse being asked for, and until now that meant *nothing happened at all*: `playFromAyah` looked for an index, found none, and returned without playing, without failing and without touching `AudioState` — the tooltip's play button was dead on every verse `ayahs` did not cover. `ayahs` tracks a single page, so in **dual-page** mode it holds only the left half of a spread (the two `LoadPage` calls a settled spread issues leave the right page's verses out of it), and the **line-accurate** renderer draws from `mushafPageLayoutCache`, a different source that can have a page fully on screen before `getAyahsByPage` has landed for it. A miss is now resolved rather than dropped: the verse's own surah is fetched and it plays from there, which is what "play from here" means everywhere else; only a verse its surah cannot account for falls back to `playAyah`, one verse alone. `playFromAyah` returns `Boolean` to make that decision the caller's — a playlist that cannot start is a fact, not a no-op. Covered by `QuranViewModelAyahPlaybackTest`.
...
**Where the bar is drawn.** Both screens that can *start* a recitation: `QuranReaderScreen` and `QuranBrowseScreen`. Browse had none, which made surah info's "Listen" read as a dead button — it starts a whole-surah playlist, and `playAyahsSequentially` downloads every verse before the first note, so on a long surah nothing was audible for a long time and the screen said nothing at all about why. Nothing could stop it either, short of opening a surah to find a player. The bar sets `isPreparing` in the same frame as the tap, so wiring it to browse turns that silence into the download strip counting up; "Listen" also closes the sheet now, which used to stand over the player it had just summoned. Coordinates come from `AudioState` alone there — no reading position to borrow, and `pageNumber`/`juzNumber` are passed 0 rather than invented, the same as the reader does for a verse off its page. On a tablet the list pane yields the player to the detail pane's reader (`showPlayer = currentArgs == null` in `AdaptiveQuranScreen`), so the two panes never draw the same player twice — but with an empty detail pane the list keeps it, which is the tablet case "Listen" would otherwise have nowhere to report in. **The whole-surah pre-download is still there**: this makes the wait legible, not shorter.
