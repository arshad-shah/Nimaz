...
- **Selection.** `SettingsRepository.quranTranslatorId` (key `quran_translator_id`, default `sahih_international`). `QuranSettingsScreen` shows a single `NimazSettingsItem` row carrying the current translator, which opens **`NimazListPicker`** — the shared modal picker sheet, the same one `PrayerSettingsScreen` uses for calculation method. It supplies search (auto-enabled past 8 items), the language grouping (via `NimazPickerItem.group`) and scroll-to-selected, so 15 translations cost one row on the settings screen rather than ~23 inlined items.
- **Live preview.** The settings preview card renders the *selected translation's actual text* for its sample ayah (`SettingsViewModel.PREVIEW_AYAH_ID` = 1, the Bismillah it already draws in Arabic), so switching translation is visible immediately. `observeQuranPreviewTranslation()` watches the persisted preference and resolves it through `GetAyahTranslationUseCase`; `flatMapLatest` keeps a slow earlier load from overwriting a newer pick, which matters because the first read of a translation also seeds its 6,236 rows. A null result keeps the previous text rather than blanking the card.
- **RTL.** The catalogue includes Urdu, so the reader cannot inherit the app's layout direction for translation text. `QuranAyahItem` and `AyahTranslationBottomSheet` set `textDirection = TextDirection.Content` with `TextAlign.Start`, resolving direction from the text itself.
