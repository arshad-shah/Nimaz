...
|---|---|---|---|
| `QuranAudioService` | `:core:audio` `data/audio/` | foreground `mediaPlayback` — Quran recitation, owns the `MediaSession` | [§1](#1-audio-playback) |
| `AdhanPlaybackService` | `:core:audio` `data/audio/` | foreground `mediaPlayback` — adhan playback when a prayer fires | [§1](#1-audio-playback) |
| `AdhanDownloadService` | `:core:audio` `data/audio/` | foreground `dataSync` — adhan file download | [§1](#1-audio-playback), [§3](#3-background-work-workmanager) |
...
| `AdhanDownloadWorker` | `:core:audio` `data/audio/` | one-shot fallback when a foreground service can't start | [§3](#3-background-work-workmanager) |
...
There are **three independent playback engines** (Quran recitation, Adhan, Qaida tap-to-hear) plus
an Adhan **download** pipeline. They share no player instance.

> **Module:** everything below except `QaidaAudioManager` is **`:core:audio`** (`core/audio/…`),
> which also brings the three `<service>` entries and the `FOREGROUND_SERVICE*` permissions in its
> own manifest. `QaidaAudioManager` stays in `:feature:content`, where its one consumer is.
>
> The module could not have been created before the channel ids moved: `AdhanPlaybackService` read
> `PrayerNotificationScheduler.CHANNEL_ID_ADHAN` while `BootReceiver` started the two adhan
> services, which is a circular project dependency once each side is a module — and one
> `moduleBoundary` would not have caught first, because both sides are `:core:*`. See
> [§0.6](#06-notification-channels).
>
> Two couplings to `:app` were unpicked with the move. `QuranAudioService` built its content
> `PendingIntent` with `Intent(this, MainActivity::class.java)`; it resolves the launcher
> *component* now, the way `:feature:widget` does. And all three services drew
> `AppR.drawable.ic_stat_nimaz`, which is **`:core:ui`'s** drawable now — it was the only one
> `:app` had, and `:core:notifications` needs it next.
...
| `data/audio/AdhanSound.kt` | `:core:data` in PR 21 of #551, `:core:audio` now — beside the manager that reads it. Enum of adhans (MISHARY, ABDUL_BASIT, MAKKAH, SIMPLE_BEEP) with per-variant file names + download URLs |

**Wiring.** The managers are `@Singleton @Inject constructor(@ApplicationContext …)` (Hilt provides them automatically) and the services are `@AndroidEntryPoint` field-injecting their manager. The four *bindings* — `QuranPlayback`, `AyahAudioDownloader`, `NextSurahPlaylistSource`, `AdhanDownloader` — are `AudioBindingsModule` in `:core:audio`, following the rule that a binding lives with its implementation; they were the last audio entries in `:app`'s `RepositoryModule`, which is down to three. Services are declared in **`:core:audio`'s** `AndroidManifest.xml` with `foregroundServiceType` `mediaPlayback`/`dataSync`, fully qualified rather than with a leading dot (a leading dot resolves against the *application* package, and a library manifest is merged into whatever consumes it); the `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_MEDIA_PLAYBACK` and `FOREGROUND_SERVICE_DATA_SYNC` permissions are declared there too, so the module is self-describing.
