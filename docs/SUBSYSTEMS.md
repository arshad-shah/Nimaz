...
- **Selection.** `SettingsRepository.quranTranslatorId` (key `quran_translator_id`, default `sahih_international`); the picker in `QuranSettingsScreen` renders `QuranTranslation.byLanguage()` as one `NimazMenuGroup` per language.
