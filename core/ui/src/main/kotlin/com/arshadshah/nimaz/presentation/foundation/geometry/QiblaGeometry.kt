...
// Public because `molecules/qibla/` stayed in `:app` and draws with these. `atoms/qibla/` moved
// here as part of #561's 52 atoms while its molecules travel with `:feature:qibla` later, so the
// two halves of the qibla UI now sit either side of a module boundary.
fun DrawScope.drawBeam(x: Float, color: Color, isFacing: Boolean) {
...
fun DrawScope.drawArcToKaaba(pointRight: Boolean, color: Color) {
