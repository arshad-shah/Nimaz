...
// The four path builders below are public for the same reason as `NimazToneColors`: they are
// generic geometry, and `SurahHeaderCartouche` — a Quran molecule still in `:app` — draws with
// them across the new module boundary.
fun scallopPath(c: Offset, r: Float, lobes: Int, anchor: Float, control: Float): Path {
    val step = 360f / lobes
    return Path().apply {
...
fun cartouchePath(x0: Float, y0: Float, x1: Float, y1: Float, t: Float): Path {
    val cy = (y0 + y1) / 2f
    val bl = x0 + t
    val br = x1 - t
    val bow = (y1 - y0) * 0.30f
...
fun diamondPath(c: Offset, r: Float): Path = Path().apply {
...
fun circlePath(c: Offset, r: Float): Path = Path().apply {
