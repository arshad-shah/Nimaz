...
/**
 * Every `XxxUseCases` aggregate, provided beside the use cases themselves.
 *
 * The second half of `:app`'s old 905-line `RepositoryModule`, moved here in PR 22 of #551. A
 * use-case aggregate is domain code and depends only on `:core:domain` repository interfaces, so
 * `:app` was never the right place to construct one — it was simply where all DI lived.
 *
 * This is the only Dagger declaration in the pure layer, and it is deliberate: `hilt-core` is the
 * JVM half of Hilt, so `androidFreeClasspath` still passes and `import android.*` is still a
 * compile error here. What the module says is *where its bindings install*, not how anything is
 * built.
 */
@Module
...
object UseCaseModule {
    ...
    fun provideQuranUseCases(
        ...
        return QuranUseCases(
            ...
            getAllTopics = GetAllTopicsUseCase(repository),
