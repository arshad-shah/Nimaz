 ...
 *
 * `internal` because `toPageLayout` takes [MushafLayoutLineRow] — a `QuranDao` projection, and so
 * a `:core:database` type — in what was a `public` signature. That is the one real entity leak in
 * this module's API, and #560's validation is right that it is the live risk, while the criterion
 * the issue leads with ("every repository returns domain models") was already true and checks
 * nothing: those repositories implement interfaces declared in `:core:domain`, a `kotlin-jvm`
 * module, so an entity in one of their signatures would not compile. Its only callers are
 * [QuranRepositoryImpl] and its own test, both here, so narrowing it costs nothing.
 * `PublicApiHasNoPersistenceTypesTest` fails if another one appears.
 */
