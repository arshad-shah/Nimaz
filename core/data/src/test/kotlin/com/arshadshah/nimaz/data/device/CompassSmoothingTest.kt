 ...
 *
 * **Moved here with its subject in PR 9 of #551**, from `presentation.viewmodel.prayer` in `:app`.
 * `smoothInto` is `internal`, and `internal` is scoped to a *module*, so the moment
 * `AndroidCompassSensors` landed in `:core:data` this test stopped compiling where it was. That is
 * the useful half of the failure: a test whose package said "presentation" was reaching into a
 * data-layer implementation detail, and nothing said so while both lived in one module. The
 * boundary is what surfaced it.
 */
