...
/**
 * Delivers every alarm the app arms: the prayer notification and its pre-reminder, the nightly
 * summary, the Friday reminder, the khatam nudge, the extended worship reminders, and the
 * midnight reschedule that arms the next day's.
 *
 * **It was the second half of `BootReceiver`**, which at 812 lines did two unrelated jobs. Boot
 * recovery is a manifest entry point pinned to `:app`; this is not. Splitting them is what lets
 * the notification machinery move to `:core:notifications` without dragging
 * `WidgetUpdateScheduler` — and therefore `:feature:widget` — up with it, which `moduleBoundary`
 * would refuse.
 *
 * Every action here arrives through an explicit `PendingIntent` armed by
 * [PrayerNotificationScheduler], so this receiver needs a manifest `<receiver>` entry and no
 * `<intent-filter>` at all.
 *
 * **Alarms armed by an older build point at `BootReceiver`, not at this class.** A rename breaks
 * a `PendingIntent` silently: the alarm fires, resolves a component that no longer handles the
 * action, and nothing happens. Two things close that window — `AppInitializer` re-arms
 * everything on each launch, and `BootReceiver` now also listens for `MY_PACKAGE_REPLACED`, so
 * the reschedule happens seconds after the update whether or not the app is opened.
 *
 * Everything here runs after `onReceive` returns, so each branch is wrapped in [goAsync] — see
 * the note on [onReceive].
 */
@AndroidEntryPoint
class PrayerAlarmReceiver : BroadcastReceiver() {
    ...
    /**
     * Dispatches one alarm, on a coroutine kept alive by [goAsync].
     *
     * **Without `goAsync` none of this was guaranteed to run.** Every branch below launches work
     * that outlives `onReceive`, and the moment `onReceive` returns Android is free to treat the
     * process as having nothing to do and kill it — most likely right after a reboot, which is
     * exactly when a re-arm matters most. The receiver instance is discarded after each call, so
     * the `CoroutineScope` that used to be a field of it was never cancelled either: a scope per
     * dispatch, cancelled when the work finishes, is both correct and shorter.
     *
     * `goAsync()` returns `null` when the receiver was not dispatched by the framework — a unit
     * test calling `onReceive` directly — so the result is treated as nullable rather than
     * asserted. There is nothing to finish in that case.
     */
    override fun onReceive(context: Context, intent: Intent) {
        ...
        scope.launch {
            try {
                dispatch(context, intent, action)
            } catch (e: Exception) {
                // dispatch() already reports per-branch; this is the last line of defence, and
                // it must not stop `finish()` being called — a PendingResult that is never
                // finished holds a wake lock until the system reclaims it.
    ...
    private suspend fun dispatch(context: Context, intent: Intent, action: String) {
        when (action) {
            ...
            PrayerNotificationScheduler.ACTION_DAILY_SUMMARY -> handleDailySummary(context)

            PrayerNotificationScheduler.ACTION_FRIDAY_REMINDER -> handleFridayReminder(context)

            PrayerNotificationScheduler.ACTION_KHATAM_REMINDER -> handleKhatamReminder(context)
    ...
    private suspend fun handlePrayerNotification(context: Context, intent: Intent) {
        val prayerName =
    ...
    private suspend fun handleDailySummary(context: Context) {
        try {
            ...
            if (!prefs.prayerNotificationsEnabled) return
     ...
     */
    private suspend fun handleFridayReminder(context: Context) {
        try {
            val prefs = preferencesDataStore.userPreferences.first()
            if (!prefs.prayerNotificationsEnabled) return
            if (!preferencesDataStore.fridayReminderEnabled.first()) return
     ...
     */
    private suspend fun handleKhatamReminder(context: Context) {
        try {
            ...
            if (!preferencesDataStore.khatamReminderEnabled.first()) return

            val khatam = khatamRepository.observeActiveKhatam().first() ?: return
     ...
     */
    private suspend fun handleWorshipReminder(context: Context, intent: Intent) {
        ...
        try {
            ...
            val prefs = preferencesDataStore.userPreferences.first()
            if (!prefs.prayerNotificationsEnabled) return
            if (!preferencesDataStore.worshipReminderEnabled(type.key).first()) return
    ...
    companion object {
        ...
        /**
         * The six actions this receiver answers to.
         *
         * Checked before [goAsync] rather than relying on the `when` falling through: a
         * `PendingResult` taken for an action nothing handles is a `PendingResult` nothing
         * finishes, which holds a wake lock until the system reclaims it. Anything else is
         * dropped without taking one.
         */
