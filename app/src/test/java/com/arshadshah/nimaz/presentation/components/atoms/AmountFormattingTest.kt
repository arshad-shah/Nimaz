...
class AmountFormattingTest {
    ...
    fun `already-grouped text re-groups to the same thing`() {
        ...
        assertThat(formatAmountInput("42,180")).isEqualTo("42,180")
        assertThat(formatAmountInput(formatAmountInput("1234567"))).isEqualTo("1,234,567")
    }
    ...
    fun `a leading point is left alone rather than grown a zero`() {
        ...
        assertThat(formatAmountInput(".5")).isEqualTo(".5")
    }

    @Test
    fun `parsing reverses the grouping`() {
        assertThat(parseAmountInput("42,180.50")).isEqualTo(42180.50)
        assertThat(parseAmountInput("1,234,567")).isEqualTo(1234567.0)
        assertThat(parseAmountInput(".5")).isEqualTo(0.5)
    }
    ...
    fun `a half-typed amount parses to the number so far, not to zero`() {
        ...
        assertThat(parseAmountInput("42,180.")).isEqualTo(42180.0)
    ...
    fun `a stored amount renders without trailing noise`() {
        ...
        assertThat(amountToInput(42180.0)).isEqualTo("42,180")
        assertThat(amountToInput(42180.5)).isEqualTo("42,180.5")
        assertThat(amountToInput(1284.55)).isEqualTo("1,284.55")
        assertThat(amountToInput(0.0)).isEqualTo("")
    }

    @Test
    fun `a stored amount round-trips through the field`() {
        for (amount in listOf(0.0, 0.5, 65.0, 1284.55, 42180.0, 1234567.89)) {
            assertThat(parseAmountInput(amountToInput(amount))).isEqualTo(amount)
        }
    }
}
